...
class StockGUI:
    def __init__(self, guiWin, api_key):
        ...
        self.guiWin_.geometry("1650x950")  ## KSF Apr 2021 - make it bigger than 930x950
        ...
        ##  KSF main2_frame  ###############################################################################################
        # Create Frame inside GUI canvas
        self.main2_frame = ttk.Frame(self.guiWin_, padding="15 15 5 15")   ## 2nd = NORTH ! wnes /or in guiWin_  instead ?  mainframe
        self.main2_frame.grid(column=1, row=0, sticky=(N, W))     ## was sticky=(N, W, E, S)
        
        
        ##  KSF frame_b  ###############################################################################################
        # Create Frame inside main2_frame canvas
        self.frame_b = ttk.Frame(self.main2_frame, padding="15 15 5 15")   ## 2nd = NORTH ! wnes /or in guiWin_  instead ?  mainframe
        self.frame_b.grid(column=1, row=0, sticky=(N, W))     ## was sticky=(N, W, E, S)
        ...
        ttk.Separator(self.frame_b, orient=HORIZONTAL).\
                               grid(column=1, row=7, columnspan=4, sticky="EW")
        ...
        ##  KSF frame_c  -- starts here  ######################################################################################
        ##  KSF frame_c  ###############################################################################################
        # Create Frame inside main2_frame canvas
        self.frame_c = ttk.Frame(self.main2_frame, padding="15 15 15 15")   ## or in guiWin_  instead ?  mainframe
        self.frame_c.grid(column=2, row=0, sticky=(N, W))     ## was sticky=(N, W, E, S)
        

        
        
        # Add Label Widgets to frame_c
        ...
        ttk.Separator(self.frame_c, orient=HORIZONTAL).\
                               grid(column=1, row=7, columnspan=4, sticky="EW")
        ...
        print("*** 339: SETUP ENTRY_WiDGEt for qtrly Rev Growth YOY")
        ...
        # # ############## end of frame_c ############    
        # # ##############  frame_d - for NARRATIVE  ############  
        # Create Frame inside main2_frame canvas
        self.input_text = StringVar() 
        
        self.frame_d = ttk.Frame(self.main2_frame, padding="5 5 5 5")   ## or in guiWin_  instead ?  mainframe
        self.frame_d.grid(column=1, row=1, sticky=(N, W))     ## was sticky=(N, W, E, S)
        
        langtickers=("English", "Spanish", "German", "French", "Italian",
             "Afrikaans", "Zulu", "Arabic", "Persian", "Chinese",
             "Japanese", "Korean", "Samoan", "Russian")
        
        self.lang_selected = StringVar()          
        self.lang_spinbox = Spinbox(self.frame_d, values=langtickers, width=10, 
                             textvariable=self.lang_selected, font=("Arial", 20), 
                             justify=CENTER, fg='white', bg='red', 
                             buttonbackground='black')  
        self.lang_spinbox.grid(column=1, row=1, sticky=W, padx=10, rowspan=1)
        self.lang_selected.set('English')  ## ??
        
        ## convert the WORD LANGUAGE to the Google CODE LANGUAGE
        Dict_Lang = {"English":"en", "Spanish":"es", "German":"de", "French":"fr", "Italian":"it",
                     "Afrikaans":"af",  "Zulu":"zu", "Arabic":"ar", "Persian":"fa",
                     "Chinese":"zh", "Japanese":"ja", "Korean":"ko", "Samoan":"sm", "Russian":"ru" }
        
        self.target = StringVar()
        ttt = self.lang_selected.get()
        CODETarget_Lang = Dict_Lang.get(ttt)  
        self.target.set(CODETarget_Lang)
        
        print("\n DICTIONARY RESULT FOR LANGUAGE:  ", ttt,"\n is CODED: ",
                      CODETarget_Lang,"\n\n")
        ##############################################################################################
        #################################  FOR ALL YEARS: TRY  SPIN BOX ##############################
        ttk.Label(self.stock_frame, text="PICK A TIMEFRAME").grid(column=1,row=4)
        ...
        ############ remove this
        # Add EXAMPLE  Label Widgets to frame_d
        # ttk.Label(self.frame_d, text="NARRATIVE GOES HERE").grid(column=1,row=1, sticky=W)
        
        # # ############## end of frame_d (Narr) ############  
        # # ############## Delete - BELOW were OLD CheckBoxes Method of selecting & running Stk ############ 
        # # ############## Delete - BELOW were OLD CheckBoxes Method of selecting & running Stk ############ 
    ...
    def ksfOVERVIEW_FrameC(self):
        ...
        try:
            ...
            ####################################3
            self.ProfitMargin_temp = self.result.get("ProfitMargin")  ########## get ProfitMargin
            self.ProfitMargin.set(self.ProfitMargin_temp)
            # self.ProfitMargin.set(self.ProfitMargin_temp+" %")
            print("** 523 -- ProfitMargin \n", self.ProfitMargin.get())
            
            self.OperatingMarginTTM_temp = self.result.get("OperatingMarginTTM")  ########## get OperatingMarginTTM
            ...
            self.ReturnOnEquityTTM_temp = self.result.get("ReturnOnEquityTTM")  ########## get ROE
            self.ReturnOnEquityTTM.set(""+self.ReturnOnEquityTTM_temp)
            print("** 532 -- ReturnOnEquityTTM \n", self.ReturnOnEquityTTM.get())
            
            self.QuarterlyRevenueGrowthYOY_temp = self.result.get("QuarterlyRevenueGrowthYOY")  ########## get Rev Gro YOY
            ...
            self.ForwardPE_temp = self.result.get("ForwardPE")  ########## get % Insiders
    ...
    def stock_stat(self) : 
        ...
        try:
            ...
            # load all the items into   frame_b: & into frame_c **********************************
    ...
    def get_series(self):
        ...
        n = nn ## len is 100 for 100 days // 249 for a Year // 2490 for 10 yrs // 0 for ALL YRS ... full
        if nn == 0:
            n = len(series)
    ...
    def lang_translate(self):
        ...
        See https://g.co/cloud/translate/v2/translate-reference#
        """
        ...
        print("923***   inside function....  lang_translate")
        ...
        temp_transl = self.translation.get()
        self.JC_co_desc.set(temp_transl)   ########### ksf & jc - loading variable *****
        print("'self.JC_co_desc' \n", temp_transl)
    ...
    def JC_DispNarr(self):
        ...
        # self.target = StringVar()
        ttt = self.lang_selected.get()
        CODETarget_Lang = Dict_Lang.get(ttt)  ## get lang CODE from Dict_Lang
        self.target.set(CODETarget_Lang)
        print("974*** after NARR Called, self.lang_selected  is:\n", ttt,
                       "CODETarget_Lang is CODED value = ", CODETarget_Lang)
        
        ## NOW CALL DISPLAY TO GET THE NARRATIVE (and later trans it)
        self.KSF_DispNarr()
        
        ############  LATER IT CALLS (DR JONES) LANG TRANS
        #self.lang_translate()
       
        # temp = 1
        """## JUAN CARLOS -- Puts his Narrative Code in Here
        ***
        ## in order to RUN JC's code - need to change (10 ines above this)
        (INSIDE the DispNarr DEFINED FUNCTION):
        
        UseJC_NARR = TRUE  (ie this line must be at top inside  def DispNarr)
        """
        
    ###############################################  
    ###############################################    
    def KSF_DispNarr(self):
        """ THIS IS KAREN's OLD NARRATIVE - 
        
        *** in order to invoke this (Rather than JC's Narrative:
            ... must set the switch (INSIDE the DispNarr DEFINED FUNCTION)
        UseJC_NARR = FALSE
        THEN THIS CODE WILL RUN
        -----------------------------------------------------------
        ### DISPLAY NARRATIVE  ... in a TEXTBOX  ### """ 
        ############### SET LOCAL SWITCH TO USE JC's NARRATIVE ****##
        UseJC_NARR = TRUE  ## or set to false to just use Karen's
        #################### CLEAR PRIOR GRAPH ####################
        self.imgwin = ttk.Label(self.mainframe, text=""). \
                grid(column=1, row=9, columnspan=4, sticky=W)
        ...
        if x     != "narrative":   
            ...
            try:    
                ...
                print("*** # 1110  keys of Dictionary",KSF_keys)
            except:
                print("***  #1121:  KSF error - this dictionary has error when try to get KEYS")
            
            try:
                self.KSF_co_desc = result.get("Description")  ################# get nar
                print("1125***Description \n", self.KSF_co_desc)
                ...
                print("***  1131:  KSF - dont need .get()\n", self.textVar)  
                # WITH .get()
                # self.textVar = self.KSF_co_desc.get()  ### KSF - do I need .get() ?
                # print("***  608  KSF - dont need .get()\n", self.textVar) 
                
            except:
                print('*** # 1137: KSF  result.get("Description")  does not work to get description') 
        ...
        if KSF_Label == 0:
            # 1st Create Text Box
            self.t = Text(self.frame_d, relief='sunken', wrap='word', width=33,
                          height=32, bg='maroon', fg='white', font=('Arial', 12))
            self.t.grid(column=1, columnspan=6, row=9, rowspan=1, 
                      sticky=(N, W, S, E))
            ...
            if UseJC_NARR:
                try:
                    ...
                    untranslatedText = self.KSF_co_desc.get()
                    
                    self.input_text.set(untranslatedText)
                    print("1159***Print 'self.input_text' \n", untranslatedText)
                    
                except:
                    untranslatedText = self.KSF_co_desc
                    
                    self.input_text.set(untranslatedText)
                    print("1165***Print 'self.input_text' \n", untranslatedText)
                    
                ## need to get the language that was set by SpinBo & then from Dict_Lang:
                tttt = self.target.get()
                print("1169***Print 'self.target'  \n", tttt)
                
                ## CALL THE TRANSLATION PROGRAM --------------------------------------
                self.lang_translate()
                done_trans = self.JC_co_desc 
                
                try:
                    self.KSF_co_desc.set(done_trans)
                except:
                    self.KSF_co_desc = done_trans
                                    
                # translated_co_desc = self.JC_co_desc.get()
                # self.t.insert(1.0, translated_co_desc)
...
root = Tk()
root.title("Karen-Monica-JuanCarlos's Stock Price GUI")
api_key         = "demo"
