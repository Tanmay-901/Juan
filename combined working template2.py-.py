...
class StockGUI:
    def __init__(self, guiWin, api_key):
        ...
        newsize =  (480, 240)
        self.boat_img = Image.open(".\Boat_Skiier.png")
        ...
        stocktickers=("IBM", "AAPL", "MSFT", "GE", "GM",   "MANT", "F", "TWTR", "BKR", "XOM",
                    "CHEV", "JNJ","GOOG",  "FB","AMZN",  "GS", "JMP", "MS", "GD", "NFLX",
                    'MRNA', 'PFE', 'NVAX', 'AZN')
        ...
        ##  KSF frame_b  ###############################################################################################
        # Create Frame inside GUI canvas
        self.frame_b = ttk.Frame(self.guiWin_, padding="15 15 15 15")   ## 2nd = NORTH ! wnes /or in guiWin_  instead ?  mainframe
        ...
        self.tr_pe_entry.grid(column=2, row=4, sticky=(W, E))
        ...
        self.payout_rat_entry.grid(column=2, row=5, sticky=(W, E))
        ...
        ##  KSF frame_c  ###############################################################################################
        # Create Frame inside GUI canvas
        self.frame_c = ttk.Frame(self.guiWin_, padding="15 15 15 15")   ## or in guiWin_  instead ?  mainframe
        ...
        self.lang_spinbox = Spinbox(self.frame_b, values=langtickers, width=10,
                             textvariable=self.lang_selected, font=("Arial", 20),
                             justify=CENTER, fg='white', bg='red',
                             buttonbackground='black')
        self.lang_spinbox.grid(column=2, row=12, sticky=W, padx=10, rowspan=1)
        self.lang_selected.set('English')



        self.langs = ["en", "es", "de", "fr", "it", "af", "zu", "ar", "fa",
                     "zh", "ja", "ko", "sm", "ru" ]
    ...
    def lang_translate(self,result):
        ...
        lang= self.lang_spinbox.get()
        driver.get("https://www.google.com/search?q=translate+english+to+{}".format(lang))
        time.sleep(3)
        g = driver.find_element_by_class_name('tw-ta.tw-text-large.XcVN5d.goog-textarea')
        g.clear()
        g.send_keys(result["Description"])
        while 1:
            try:
                result = driver.find_element_by_class_name('tw-data-text.tw-text-large.XcVN5d.tw-ta').text
                break
            except:
                print("Trying...")
        ...
        # result = translate_client.translate(target_language=self.target)
    ...
    def lang2(self):
        self.lang_clear()
        self.l2.set(1)
        self.target=self.langs[1]
    def lang3(self):
        self.lang_clear()
        self.l3.set(1)
        self.target=self.langs[2]
    def lang4(self):
        self.lang_clear()
        self.l4.set(1)
        self.target=self.langs[3]
    def lang5(self):
        self.lang_clear()
        self.l1.set(1)
        self.target=self.langs[4]
    def lang6(self):
        self.lang_clear()
        self.l2.set(1)
        self.target=self.langs[5]
    def lang7(self):
        self.lang_clear()
        self.l3.set(1)
        self.target=self.langs[6]
    def lang8(self):
        self.lang_clear()
        self.l4.set(1)
        self.target=self.langs[7]
    def lang9(self):
        self.lang_clear()
        self.l1.set(1)
        self.target=self.langs[8]
    def lang10(self):
        self.lang_clear()
        self.l2.set(1)
        self.target=self.langs[9]
    def lang11(self):
        self.lang_clear()
        self.l3.set(1)
        self.target=self.langs[10]
    def lang12(self):
        self.lang_clear()
        self.l4.set(1)
        self.target=self.langs[11]
    def lang13(self):
        self.lang_clear()
        self.l1.set(1)
        self.target=self.langs[12]
    def lang14(self):
        self.lang_clear()
        self.l2.set(1)
        self.target=self.langs[13]

    def read_txt(self):
        with open("narrative.txt","r")as g:
            g.write(result["Description"])
            g.close()
